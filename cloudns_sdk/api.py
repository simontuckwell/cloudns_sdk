...
class ClouDNSAPI:
    ...
    def add_cloud_domain(self, domain_name, cloud_domain_name):
        ...
        return self.make_request('dns/add-cloud-domain.json', method='POST', params=params)

    def delete_cloud_domain(self, domain_name):
        params = self._auth_params({'domain-name': domain_name})
        return self.make_request('dns/delete-cloud-domain.json', method='POST', params=params)

    def change_cloud_master(self, domain_name):
        params = self._auth_params({'domain-name': domain_name})
        return self.make_request('dns/set-master-cloud-domain.json', method='POST', params=params)

    def list_cloud_domains(self, domain_name):
        params = self._auth_params({'domain-name': domain_name})
        return self.make_request('dns/list-cloud-domains.json', method='POST', params=params)
